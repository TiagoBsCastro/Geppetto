#!/bin/bash
#SBATCH --job-name=components_n2160_fullsky
...
#SBATCH --exclusive
#SBATCH --output=logs/components_%A_%a.out
#SBATCH --error=logs/components_%A_%a.err
...
srun --ntasks=1 --cpus-per-task="${SLURM_CPUS_PER_TASK}" --cpu-bind=none \
	python "${REPO}/examples/measure_fullsky_component_power.py" \
	--manifest "${RUN}/geppetto_reduced/painted_nfw_manifest.csv" \
	--observed-cache "${RUN}/geppetto_reduced/fullsky_observed_spectra.npz" \
	--output "${RUN}/geppetto_reduced/fullsky_component_spectra.npz"
